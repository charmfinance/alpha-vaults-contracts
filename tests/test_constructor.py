...
def test_constructor(PassiveRebalanceVault, pool, gov):
    vault = gov.deploy(
        PassiveRebalanceVault, pool, 2400, 1200, 500, 600, 100e18
    )
...
def test_constructor_checks(PassiveRebalanceVault, pool, gov):
    with reverts("threshold not tick multiple"):
        gov.deploy(
            PassiveRebalanceVault, pool, 2401, 1200, 500, 600, 100e18
        )

    with reverts("threshold not tick multiple"):
        gov.deploy(
            PassiveRebalanceVault, pool, 2400, 1201, 500, 600, 100e18
        )
    ...
    with reverts("threshold too high"):
        gov.deploy(
            PassiveRebalanceVault, pool, 887280, 1200, 500, 600, 100e18
        )

    with reverts("threshold too high"):
        gov.deploy(
            PassiveRebalanceVault, pool, 2400, 887280, 500, 600, 100e18
        )
    ...
    with reverts("maxTwapDeviation"):
        gov.deploy(
            PassiveRebalanceVault, pool, 2400, 1200, -1, 600, 100e18
        )
