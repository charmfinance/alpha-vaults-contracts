...
def test_constructor(PassiveRebalanceVault, pool, gov):
    vault = gov.deploy(PassiveRebalanceVault, pool, 2400, 1200, 500, 600, 10000, 100e18)
...
def test_constructor_checks(PassiveRebalanceVault, pool, gov):
    with reverts("threshold not tick multiple"):
        gov.deploy(PassiveRebalanceVault, pool, 2401, 1200, 500, 600, 10000, 100e18)

    with reverts("threshold not tick multiple"):
        gov.deploy(PassiveRebalanceVault, pool, 2400, 1201, 500, 600, 10000, 100e18)

    with reverts("threshold not positive"):
        gov.deploy(PassiveRebalanceVault, pool, 0, 1200, 500, 600, 10000, 100e18)

    with reverts("threshold not positive"):
        gov.deploy(PassiveRebalanceVault, pool, 2400, 0, 500, 600, 10000, 100e18)

    with reverts("threshold too high"):
        gov.deploy(PassiveRebalanceVault, pool, 887280, 1200, 500, 600, 10000, 100e18)

    with reverts("threshold too high"):
        gov.deploy(PassiveRebalanceVault, pool, 2400, 887280, 500, 600, 10000, 100e18)

    with reverts("maxTwapDeviation"):
        gov.deploy(PassiveRebalanceVault, pool, 2400, 1200, -1, 600, 10000, 100e18)

    with reverts("twapDuration"):
        gov.deploy(PassiveRebalanceVault, pool, 2400, 1200, 500, 0, 10000, 100e18)

    with reverts("protocolFee"):
        gov.deploy(PassiveRebalanceVault, pool, 2400, 1200, 500, 600, 1e6, 100e18)
